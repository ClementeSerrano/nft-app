...
const getDBInstance = async (
  dataPath = '~/.lisk/nft-app/',
  dbName = 'nft_plugin.db'
) => {
  const dirPath = path.join(
    dataPath.replace('~', os.homedir()),
    'plugins/data',
    dbName
  );
...
const saveTransactions = async (db, payload) => {
  ...
  const encodedTransactions = codec.encode(encodedTransactionSchema, {
    transactions,
  });
...
const getTransactions = async (db) => {
  try {
    ...
    const { transactions } = codec.decode(
...
const getNFTHistory = async (db, dbKey) => {
  try {
    ...
    const { nftHistory } = codec.decode(
      encodedNFTHistorySchema,
      encodedNFTHistory
    );
  ...
  decodedBlock.payload.map(async (trx) => {
    ...
    if (module.name === 'nft') {
      ...
      if (trx.assetID === CREATENFT_ASSET_ID) {
          ...
          for (let i = 0; i < val.length; i++) {
            ...
            if (val[i].ownerAddress === senderAdress.toString('hex')) {
              ...
              if (savedHistory && savedHistory.length < 1) {
                ...
                nftHistory = [
                  Buffer.from(base32Address, 'binary'),
                  ...savedHistory,
                ];
                encodedNFTHistory = codec.encode(encodedNFTHistorySchema, {
                  nftHistory,
                });
        ...
        base32Address =
          trx.assetID === TRANSFERNFT_ASSET_ID
            ? cryptography.getBase32AddressFromAddress(
                Buffer.from(trx.asset.recipient, 'hex')
              )
...
const getTransactionAssetSchema = (transaction, registeredSchema) => {
  const txAssetSchema = registeredSchema.transactionsAssets.find(
    (assetSchema) =>
      ...
      assetSchema.assetID === transaction.assetID
  );
  if (!txAssetSchema) {
    throw new Error(
